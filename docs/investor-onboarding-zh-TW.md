...
| **費用專戶（Fee account）** | 另一個獨立子帳（建議名稱 `fee`）。季結算後管理方將應付費用對應的 **USDC／USDT** 劃轉至此供**對帳**；你確認無誤後，自**主帳**提幣至管理方指定地址。 |
...
4. 從主帳把資金劃到各**策略子帳**  
5. 在每個**策略子帳**建 API Key（**account:read + trade:read_write + wallet:read_write**；供管理方季末透過 API 劃轉 USDC/USDT 至 Fee 專戶）  
...
- 策略子帳 Key 會開 **Wallet** 權限，供管理方季末透過 API 將 USDC/USDT **內部劃轉**至 Fee 專戶。
- 對帳、看績效比較清楚。
...
4. 依管理方給你的清單建立子帳，**名稱建議**如下（方便雙方對照）：
   ...
   **Fee 專戶**：不跑策略、不放交易保證金；季結算後管理方會把應付費用對應的穩定幣**劃轉**進來供雙方對帳。詳見《投資人分潤與計費說明書》第五節。
...
| B5 | Wallet | **read_write** ← 季末管理方透過 API 劃轉 USDC/USDT 至 Fee 專戶 |
| B6 | Custody | **none** |
...
管理費、績效費**不會**從策略子帳自動扣款。請另建 **`fee` 子帳**，作為季結算後的**對帳專戶**。
...
3. 季結算流程（投資人端重點）：
   1. 管理方出具帳單；若有獲利現貨，管理方會先在策略子帳將對應部分 **TRADE 成 USDC 或 USDT**，再以**策略子帳 API**（Wallet=read_write）**Internal transfer** 至 `fee` 子帳。  
...
□ 策略子帳已開 account:read + trade:read_write + wallet:read_write
...
**Q：管理費／績效費怎麼付？**  
A：季結算後管理方開帳單；若有獲利現貨，管理方會先 TRADE 成 USDC／USDT 並劃至 Fee 專戶供對帳。你確認金額無誤後，從**主帳**提幣至管理方指定地址。**不會**從策略子帳自動扣。詳見《投資人分潤與計費說明書》。

**Q：下拉選單只有 none / read / read_write 怎麼選？**  
A：策略子帳：**Account = read**、**Trade = read_write**、**Wallet = read_write**（其餘 none）。Fee 專戶：**Account = read**、**Wallet = none**、**Trade = none**。

**Q：一定要先選 Deribit-generated key 嗎？**  
A：是。先選金鑰類型，**之後**才會出現六個權限下拉選單；不要跳過類型選擇就直接找權限頁。

**Q：策略子帳為什麼要開 Wallet？**  
A：季結算時管理方需透過 API 將 USDC/USDT **內部劃轉**至 Fee 專戶；此權限僅限該策略子帳，**不含**主帳提幣（主帳 API 未交付）。

**Q：Fee 專戶為什麼不開 Wallet？**  
A：Fee 專戶僅供對帳讀餘額（Account=read 即可）；劃轉由策略子帳 API 完成，實際付費由你自主帳提幣。

...
| **費用專戶（Fee account）** | 另一個獨立子帳（Deribit 建議名稱 `fee_acc`，至少 5 字元；勿用 `fee`）。季結算後管理方在策略子帳**先賣獲利現貨兌 USDC／USDT**，再**劃轉**至此供**對帳**；你確認無誤後，自**主帳**提幣至管理方指定地址。 |
...
4. 從主帳把資金劃到各**策略子帳**  
5. 在每個**策略子帳**建 API Key（**account:read + trade:read_write + wallet:read_write**；Trade 供季末**賣現貨兌 USDC／USDT**，Wallet 供**劃轉至 `fee_acc`**）  
6. 在 **Fee 專戶**建 API Key（**account:read**；**Wallet、Trade 均 none**），交給管理方查餘額、對帳用  
...
- 策略子帳 Key 需開 **Trade + Wallet**：季結算抽 fee 時，管理方會先在策略子帳 **Trade 賣出獲利現貨**（如 covered_call 的 BTC／ETH）兌成 USDC／USDT，再以 **Wallet 內部劃轉**至 `fee_acc`。
- 對帳、看績效比較清楚。
...
4. 依管理方給你的清單建立子帳，**名稱建議**如下（方便雙方對照）：
   ...
   **Deribit 子帳名稱至少 5 字元**（英數與底線）。`fee` 僅 3 字元會被拒絕；上表策略名稱（如 `naked`）已符合。管理方本機設定檔仍為 `accounts/.env.fee`，與 Deribit 顯示名稱無需相同（API Key 建在對應子帳即可）。

   若你只開其中一、兩個策略，只建對應的**策略**子帳即可；**Fee 專戶仍建議一律建立**（即使尚未有應付費用，餘額可為 0）。

   **Fee 專戶**：不跑策略、不放交易保證金；季結算後管理方在策略子帳**賣現貨 → 劃轉穩定幣**進來供雙方對帳。詳見《投資人分潤與計費說明書》第五節。
...
管理方策略以 **Segregated Portfolio Margin** 運作。請對**主帳戶**以及**每一個**子帳（含 `fee_acc`）完成設定：
...
### 5.0 季結算抽 fee：為什麼 Trade 與 Wallet 都要開？

管理費／績效費**不會**自動從策略子帳扣款。季結算後，管理方在**策略子帳**用 API 完成兩步（再劃至 `fee_acc` 供對帳）：

1. **Trade（read_write）** — 若有獲利**現貨**（covered_call 的 BTC／ETH 最常見），先在策略子帳**賣出**兌成 USDC 或 USDT。  
2. **Wallet（read_write）** — 將兌換後的 USDC／USDT（或子帳上已有的穩定幣）**Internal transfer** 至 `fee_acc`。

因此策略子帳的 **Trade 不只是期權 bot 下單**，也包含季末**賣現貨抽 fee**；**Wallet 則負責劃轉至 `fee_acc`**。Fee 專戶本身**不開** Trade／Wallet。

### 5.1 一定要先「進入」該策略子帳

API Key 綁在**當前所在帳戶**上：

1. 切換到策略子帳（例如 `naked`），確認**不是** Main account、**不是** `fee_acc`。  
2. 再建立 Key。
...
| B4 | Trade | **read_write** ← 期權下單／平倉；季末**賣現貨**兌 USDC／USDT（抽 fee 前） |
| B5 | Wallet | **read_write** ← 季末將 USDC／USDT **劃轉至 `fee_acc`**（賣現貨後或子帳已有穩定幣時） |
| B6 | Custody | **none** |
| B7 | Features | 留空／預設 |

![策略子帳 Change API Key Scope（Account=read、Trade=read_write、Wallet=read_write）](./img/onboarding/10-api-strategy-permissions.png)
...
管理費、績效費**不會**從策略子帳自動扣款。請另建 **`fee_acc` 子帳**，作為季結算後的**對帳專戶**。

### 6.1 建立 Fee 專戶

1. 與第三節相同路徑建立子帳，名稱建議 **`fee_acc`**（至少 5 字元；勿用 `fee`）。  
2. 平常餘額可為 0；**不必**把策略保證金劃進 Fee 專戶。  
3. 季結算流程（投資人端重點）：
   1. 管理方出具帳單；若有獲利現貨，管理方會先在策略子帳將對應部分 **TRADE 成 USDC 或 USDT**，再以**策略子帳 API**（Wallet=read_write）**Internal transfer** 至 `fee_acc` 子帳。  
   2. 雙方對帳：你在 Deribit 後台確認 Fee 專戶餘額與帳單一致；管理方亦透過 Fee 專戶 API 核對。  
   3. 確認無誤後，你從**主帳**以 **Withdraw** 提幣至管理方指定的鏈上地址（幣別、鏈、金額以帳單與管理方指示為準）。

### 6.2 Fee 專戶 API Key：畫面順序（與策略子帳相同流程）

1. **切換到 `fee_acc` 子帳**（確認不是策略子帳）。  
2. **Account → API** → **Add new key**。  
3. **步驟 A**：選 **Deribit-generated key** → 繼續。  
4. **步驟 B** — **Change API Key Scope**（與第五節同一視窗，但選項不同）：
   ...
   | B5 | Wallet | **none**（此帳僅供對帳讀餘額，不劃轉） |
   | B6 | Custody | **none** |
   | B7 | Features | 留空／預設 |

   **Name** 若另顯示，建議填 `fee-collection`；**IP Whitelist** 依管理方指示或留空。

5. **步驟 C**：**Confirm** → 保存 Client ID / Secret → **單獨**交給管理方（勿與策略 Key 混在同一則訊息）。

![Fee 專戶 Change API Key Scope（Account=read、Wallet=none、Trade=none）](./img/onboarding/13-api-fee-permissions.png)
...
| Account | read | read |
| Trade | read_write（期權 + **季末賣現貨抽 fee**） | none |
| Wallet | **read_write**（**劃轉至 `fee_acc`**） | **none** |
...
API Key：________________
API Secret：________________
備註：僅 Account=read，未開 Wallet／Trade
```

### 6.4 你應知道的事

- Fee 專戶裡的穩定幣 = **已結算、供雙方對帳**的應付費用；策略交易仍在策略子帳進行。  
- **實際付費**由你從**主帳**提幣至管理方指定地址完成，Fee 專戶本身不對外提幣。  
- 若 Fee 專戶 API 外洩，風險限於該專戶餘額；故**勿**把大額策略資金長期放在 Fee 專戶。  
...
【保證金模式】
□ 主帳與所有子帳（含 fee_acc）均已設為 Segregated Portfolio Margin
...
【Fee 費用專戶】
子帳名稱：________（建議 fee_acc）
□ 已建立子帳
□ 已交付 Fee 專戶 API（Account=read，Wallet／Trade 均 none）

【API Key — 策略子帳】
□ 已透過安全管道交付（策略共 ____ 組）
□ 策略子帳已開 account:read + trade:read_write + wallet:read_write（Trade＝期權＋季末賣現貨；Wallet＝劃轉至 fee_acc）
...
| 請勿 | 原因 |
|------|------|
| 把主帳 API Key 交給管理方 | 風險範圍過大（含主帳提幣） |
| 策略子帳 API Key 外洩 | 含 Wallet 權限，外洩可能被對外提幣；僅透過安全管道交付、勿明文傳 Secret |
...
**Q：管理費／績效費怎麼付？**  
A：季結算後管理方開帳單；在策略子帳**先 Trade 賣獲利現貨兌 USDC／USDT**（若需要），再 **Wallet 劃至 `fee_acc`** 供對帳。你確認金額無誤後，從**主帳**提幣至管理方指定地址。**不會**從策略子帳自動扣。詳見《投資人分潤與計費說明書》。

**Q：下拉選單只有 none / read / read_write 怎麼選？**  
A：策略子帳：**Account = read**、**Trade = read_write**（期權 + 季末賣現貨）、**Wallet = read_write**（劃轉至 `fee_acc`；其餘 none）。Fee 專戶：**Account = read**、**Wallet = none**、**Trade = none**。

**Q：一定要先選 Deribit-generated key 嗎？**  
A：是。先選金鑰類型，**之後**才會出現 **Change API Key Scope** 視窗；不要跳過類型選擇就直接找權限設定。

**Q：策略子帳為什麼要開 Trade 和 Wallet？**  
A：**Trade** — 期權 bot 下單／平倉，以及季結算時**賣出獲利現貨**兌 USDC／USDT（covered_call 等策略抽 fee 前常需要）。**Wallet** — 將兌換後的穩定幣**內部劃轉**至 `fee_acc`。兩者僅限該策略子帳，**不含**主帳提幣（主帳 API 未交付）。

**Q：Fee 專戶為什麼不開 Trade／Wallet？**  
A：Fee 專戶僅供對帳讀餘額（Account=read 即可）；**賣現貨與劃轉**都在策略子帳 API 完成，實際付費由你自主帳提幣。

**Q：程式會幫我從交易所提幣到銀行嗎？**  
A：不會。管理方無主帳 API；季末付費由你從**主帳**自行提幣至管理方指定地址。

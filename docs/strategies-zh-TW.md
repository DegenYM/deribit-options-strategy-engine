...
- **快速拉升暫停開倉**：24h 或 48h 指數漲幅超過 `INDEX_RALLY_24H_PCT` / `INDEX_RALLY_48H_PCT` 時，該標的標成 `elevated`，沿用既有「非 normal 不開新倉」閘門。這不是 `crisis`，**不會**因此 hard-derisk 既有倉。策略骨架（`config/shared/strategies/.env.*`）已寫入 `ENABLE_INDEX_RALLY_ENTRY_HALT=true`、`INDEX_RALLY_24H_PCT=0.05`、`INDEX_RALLY_48H_PCT=0.07`

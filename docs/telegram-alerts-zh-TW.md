...
建議放在 **`config/shared/defaults.env`**（gitignore，全投資人共用）：
...
範本：[`config/shared/defaults.env.example`](../config/shared/defaults.env.example)（Telegram 區塊；實際金鑰寫入 gitignore 的 `defaults.env`）。

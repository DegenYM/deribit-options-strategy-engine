...
| **8. 連線測試**                          | **先** `./bot investor frontend start`（或手動 `./bot --investor … frontend --port …`），**再**執行 `cloudflared tunnel --config ~/.cloudflared/config.yml run`（見「五、Tunnel run」）。瀏覽器開 `https://<hostname>/` 或 `https://<hostname>/investor.html`。 |
| **9. Access（強烈建議）**                  | Cloudflare Zero Trust → Access → 新增 **Application**；Domain 填該投資人的 hostname（多人則每人一個 Application、各自允許名單）。檢查清單：[`cloudflare-access-checklist-zh-TW.md`](cloudflare-access-checklist-zh-TW.md) |
| **10. 開機常駐**                         | 每位投資人一個 frontend LaunchAgent + **一個** cloudflared LaunchAgent：`./bot investor frontend start` 後 `./bot investor tunnel start`（見「六、macOS launchd」）。                                                                                            |
...
**順序**：先啟動本機 **frontend**，再啟動 **tunnel**。Tunnel 502 最常見原因是 frontend 尚未監聽對應埠。

### 5a. 啟動 frontend

```bash
# 建議：依 registry.toml 的 frontend_enabled 一次啟全部
./bot investor frontend start

# 或單一投資人、指定埠（須與 config.yml ingress 一致）
./bot --investor youming frontend --port 8765
```
...
在**另一個終端機**執行（會佔住該視窗，Ctrl+C 可停止）：

```bash
cloudflared tunnel --config ~/.cloudflared/config.yml run
```

等同於 plist / launchd 裡的 `ProgramArguments`：`cloudflared tunnel --config <path> run`。

若 `config.yml` 在預設路徑，也可簡寫：

```bash
cloudflared tunnel run
```

（仍須已寫好 `~/.cloudflared/config.yml` 的 `tunnel:`、`credentials-file:`、`ingress:`。）

### 5c. 用瀏覽器驗證

frontend 與 tunnel 都在跑時，開：
...
**啟動順序**：先 `./bot investor frontend start` → 確認各 `http://127.0.0.1:<port>/api/health` → 再 `./bot investor tunnel start`。重開 Mac 後若 tunnel 先起、frontend 較晚起，短暫 502 屬預期。

### 6a. cloudflared（建議：bot 一鍵）

讀 `config/platform/registry.toml` 的 `[platform].tunnel_name`（例如 `debopt-jack`），產生並安裝 LaunchAgent `com.deribit.cloudflared.<tunnel_name>`。實際執行的仍是：

`cloudflared tunnel --config ~/.cloudflared/config.yml run`

**前置**：`~/.cloudflared/config.yml` 已存在且 `ingress` 與各投資人 `frontend_port` 一致。

```bash
./bot investor tunnel start      # 安裝 plist → launchd bootstrap → 探測本機 metrics
./bot investor tunnel stop       # bootout LaunchAgent
./bot investor tunnel restart    # 重載 plist 並 kickstart
./bot investor tunnel status     # launchd 是否 loaded + http://127.0.0.1:20241/metrics

# 等同包裝腳本
./scripts/tunnel_launchd.sh start
./scripts/tunnel_launchd.sh restart
```

| 項目 | 說明 |
|------|------|
| 產物 plist | `config/platform/generated/launchd/com.deribit.cloudflared.<tunnel_name>.plist` |
| 安裝位置 | `~/Library/LaunchAgents/` |
| 範本 | [`config/launchd/com.deribit.cloudflared.plist.template`](../config/launchd/com.deribit.cloudflared.plist.template) |
| 日誌 | `~/Library/Logs/cloudflared-<tunnel_name>.log`（stderr 同目錄 `.err.log`） |
| `status` 健康檢查 | 本機 `http://127.0.0.1:20241/metrics`；略過：`--no-health` |

改 tunnel 名稱時更新 `registry.toml` 的 `tunnel_name` 後再 `./bot investor tunnel restart`。

### 6a-alt. cloudflared（手寫 plist，可選）

若不用 `./bot investor tunnel`，可手寫 LaunchAgent。存成 `~/Library/LaunchAgents/com.example.cloudflared.deribit-tunnel.plist`：

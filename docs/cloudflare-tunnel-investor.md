...
- `[/investor.html](../frontend/investor.html)`（英文摘要）
...
| **2. 本機驗證**                          | `cd` 到 repo，`pip install -r requirements.txt`；每位對外投資人各一埠，例如：`./bot --investor alice frontend --port 8765`。瀏覽器開 `http://127.0.0.1:8765/investor.zh.html` 確認有畫面。多一位投資人就多一個終端機、不同 `--port`（8766、8767…）。 |
...
| **5. 建立 Named Tunnel**               | `cloudflared tunnel login`（選帳號與網域）→ `cloudflared tunnel create deribit-investor`（名稱自訂）。記下 **Tunnel UUID**；憑證約在 `~/.cloudflared/<UUID>.json`，執行 `chmod 600 ~/.cloudflared/*.json`。                    |
...
| **7. DNS**                           | 每個 hostname 各綁一次：`cloudflared tunnel route dns deribit-investor <hostname>`；或在 Cloudflare DNS 手動 CNAME（以官方文件為準）。                                                                                     |
| **8. 連線測試**                          | **先**啟動所有 `./bot --investor … frontend --port …`，**再**執行 `cloudflared tunnel --config ~/.cloudflared/config.yml run`。瀏覽器開 `https://<hostname>/investor.zh.html`。                                     |
...
完成後，給投資人的書籤範例：`https://alice.portfolio.example.com/investor.zh.html`（依你的 hostname 替換）。
...
2. Mac mini 上已能本機啟動 dashboard，例如：
  ...
  ```
   確認瀏覽器開 `http://127.0.0.1:8765/investor.zh.html` 有畫面。
...
cloudflared tunnel create deribit-investor
```
...
（將 `deribit-investor`、`portfolio.example.com` 改成你的 tunnel 名稱與 hostname。）

```bash
cloudflared tunnel route dns deribit-investor portfolio.example.com
```
...
`https://portfolio.example.com/investor.zh.html`

若只要內部 ops 全功能頁，可用 `https://portfolio.example.com/`（`index.html`）；**對外給投資人請只分享 investor 路徑**，並務必搭配下方 **Access**（見資安說明）。

### 讓根路徑 `/` 導向繁中投戶頁（可選）

不需改程式，可在 Cloudflare 使用 **Bulk Redirects** 或 **Redirect Rules**，將 `https://portfolio.example.com/` **301** 到 `https://portfolio.example.com/investor.zh.html`。
...
存成 `~/Library/LaunchAgents/com.example.cloudflared.deribit-investor.plist`：
...
<dict>
  <key>Label</key>
  <string>com.example.cloudflared.deribit-investor</string>
  ...
  <string>/Users/YOUR_USER/Library/Logs/cloudflared-deribit-investor.log</string>
  <key>StandardErrorPath</key>
  <string>/Users/YOUR_USER/Library/Logs/cloudflared-deribit-investor.err.log</string>
</dict>
</plist>
```
...
```bash
launchctl load ~/Library/LaunchAgents/com.example.cloudflared.deribit-investor.plist
```

...
def load_config(
    ...
    *,
    ...
    return parse_bot_config(values, require_private=require_private)


def load_config_from_values(
    values: dict[str, str],
    *,
    require_private: bool = False,
) -> BotConfig:
    """Build BotConfig from an already-merged env mapping (SaaS workers / tests)."""
    merged = {str(k): str(v) for k, v in values.items() if v is not None and str(v).strip() != ""}
    if "OPTION_STRATEGY" not in merged:
        merged["OPTION_STRATEGY"] = "covered_call"
    merged["OPTION_STRATEGY"] = _option_strategy(merged["OPTION_STRATEGY"])
    return parse_bot_config(merged, require_private=require_private)


def parse_bot_config(values: dict[str, str], *, require_private: bool = False) -> BotConfig:
